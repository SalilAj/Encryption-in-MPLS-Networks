...
struct serverrec_database *
serverrec_database_index_init_row(struct ovsdb_idl *idl, const struct ovsdb_idl_table_class *class_)
{
    return (struct serverrec_database *) ovsdb_idl_index_init_row(idl, class_);
}

/*  This function is used to compare "serverrec_database" records on table in iteration loops for compound-index operations.
    ...
    Parameters: struct ovsdb_idl_index_cursor *cursor. Cursor used to iterate over the indexed data on this table.
                const struct "serverrec_database" *const_data1,  const struct "serverrec_database" *const_data2. Data to be compared.
    Return value: 0 if both data values are equal, -1 if first parameter is less than second and 1 otherwise. */
int
serverrec_database_index_compare(struct ovsdb_idl_index_cursor *cursor, const struct serverrec_database *const_data1,  const struct serverrec_database *const_data2)
{
    struct serverrec_database *data1 = CONST_CAST(struct serverrec_database *, const_data1);
    struct serverrec_database *data2 = CONST_CAST(struct serverrec_database *, const_data2);
    return ovsdb_idl_index_compare(cursor, &data1->header_, &data2->header_);
}

/*  This function is called to position the cursor at the first row in "serverrec_database" table on the associated compound-index.
    Parameters: struct ovsdb_idl_index_cursor *cursor. Cursor used to iterate over the indexed data on this table.
    Return value: The first row in the corresponding index. */
const struct serverrec_database *
serverrec_database_index_first(struct ovsdb_idl_index_cursor *cursor)
{
    return serverrec_database_cast(ovsdb_idl_index_first(cursor));
}

/*  This function is called to position the cursor at the next row in "serverrec_database" table on the associated compound-index.
    Parameters: struct ovsdb_idl_index_cursor *cursor. Cursor used to iterate over the indexed data on this table.
    Return value: The next row in the corresponding index. */
const struct serverrec_database *
serverrec_database_index_next(struct ovsdb_idl_index_cursor *cursor)
{
    return serverrec_database_cast(ovsdb_idl_index_next(cursor));
}

/*  This function is used to find the data of the row in "serverrec_database" table that meet criteria with the requested data
    associated in the compound-index.
    Parameters: struct ovsdb_idl_index_cursor *cursor. Cursor used to iterate over the indexed data on this table.
                const struct serverrec_database *const_data. Data to be searched.
    Return value: The row in the corresponding index if found or NULL otherwise. */
const struct serverrec_database *
serverrec_database_index_find(struct ovsdb_idl_index_cursor *cursor, const struct serverrec_database *const_data)
{
    struct serverrec_database *data = CONST_CAST(struct serverrec_database *, const_data);
    return serverrec_database_cast(ovsdb_idl_index_find(cursor, &data->header_));
}

/*  This function is used to set the cursor pointing to the row in "serverrec_database" table that meet criteria of the requested data
    associated in the compound-index.
    Parameters: struct ovsdb_idl_index_cursor *cursor. Cursor used to iterate over the indexed data on this table.
                const struct serverrec_database *const_data. Data to be searched.
    Return value: The row in the corresponding index closest to the criteria. */
const struct serverrec_database *
serverrec_database_index_forward_to(struct ovsdb_idl_index_cursor *cursor, const struct serverrec_database *const_data)
{
    struct serverrec_database *data = CONST_CAST(struct serverrec_database *, const_data);
    return serverrec_database_cast(ovsdb_idl_index_forward_to(cursor, &data->header_));
}

/*  This function is used to get the data of the row in the current position pointed by the cursor in
    "serverrec_database" table.
    Parameters: struct ovsdb_idl_index_cursor *cursor. Cursor used to iterate over the indexed data on this table.
    Return value: The row in the corresponding index if found or NULL otherwise. */
const struct serverrec_database *
serverrec_database_index_get_data(const struct ovsdb_idl_index_cursor *cursor)
{
    return serverrec_database_cast(ovsdb_idl_index_data(CONST_CAST(struct ovsdb_idl_index_cursor *, cursor)));
}
...
serverrec_database_index_set_cid(const struct serverrec_database *row, const struct uuid *cid, size_t n_cid)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write_(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &serverrec_database_columns[ SERVERREC_DATABASE_COL_CID ], &datum, &serverrec_table_classes[SERVERREC_TABLE_DATABASE]);
}
...
serverrec_database_index_set_connected(const struct serverrec_database *row, bool connected)
{
    ...
    key->boolean = connected;
    datum.values = NULL;
    ovsdb_idl_index_write_(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &serverrec_database_columns[ SERVERREC_DATABASE_COL_CONNECTED ], &datum, &serverrec_table_classes[SERVERREC_TABLE_DATABASE]);
}
...
serverrec_database_index_set_index(const struct serverrec_database *row, const int64_t *index, size_t n_index)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write_(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &serverrec_database_columns[ SERVERREC_DATABASE_COL_INDEX ], &datum, &serverrec_table_classes[SERVERREC_TABLE_DATABASE]);
}
...
serverrec_database_index_set_leader(const struct serverrec_database *row, bool leader)
{
    ...
    key->boolean = leader;
    datum.values = NULL;
    ovsdb_idl_index_write_(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &serverrec_database_columns[ SERVERREC_DATABASE_COL_LEADER ], &datum, &serverrec_table_classes[SERVERREC_TABLE_DATABASE]);
}
...
serverrec_database_index_set_model(const struct serverrec_database *row, const char *model)
{
    ...
    key->string = CONST_CAST(char *, model);
    datum.values = NULL;
    ovsdb_idl_index_write_(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &serverrec_database_columns[ SERVERREC_DATABASE_COL_MODEL ], &datum, &serverrec_table_classes[SERVERREC_TABLE_DATABASE]);
}
...
serverrec_database_index_set_name(const struct serverrec_database *row, const char *name)
{
    ...
    key->string = CONST_CAST(char *, name);
    datum.values = NULL;
    ovsdb_idl_index_write_(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &serverrec_database_columns[ SERVERREC_DATABASE_COL_NAME ], &datum, &serverrec_table_classes[SERVERREC_TABLE_DATABASE]);
}
...
serverrec_database_index_set_schema(const struct serverrec_database *row, const char *schema)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write_(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &serverrec_database_columns[ SERVERREC_DATABASE_COL_SCHEMA ], &datum, &serverrec_table_classes[SERVERREC_TABLE_DATABASE]);
}
...
serverrec_database_index_set_sid(const struct serverrec_database *row, const struct uuid *sid, size_t n_sid)
{
    ...
    datum.values = NULL;
    ovsdb_idl_index_write_(CONST_CAST(struct ovsdb_idl_row *, &row->header_), &serverrec_database_columns[ SERVERREC_DATABASE_COL_SID ], &datum, &serverrec_table_classes[SERVERREC_TABLE_DATABASE]);
}
